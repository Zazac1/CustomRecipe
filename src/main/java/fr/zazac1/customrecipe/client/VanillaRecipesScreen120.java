...
public class VanillaRecipesScreen120 extends Screen {
    ...
    private Text statusFilterLabel() {
        return switch (statusFilter) {
            ...
            case ENABLED -> Text.literal("Show: Enabled").styled(s -> s.withColor(0x55FF55));
            case DISABLED -> Text.literal("Show: Disabled").styled(s -> s.withColor(0xFF5555));
        };
    }
    ...
    public VanillaRecipesScreen120(ConfigScreen parent) {
        this(parent, false);
    }

    /** Local ModMenu mode reads the vanilla recipe data already loaded by the client. */
    public VanillaRecipesScreen120(ConfigScreen parent, boolean localMode) {
    ...
    protected void init() {
        ...
        for (int i = 0; i < visibleRows && scroll + i < shownRecipes.size(); i++) {
            ...
            int y = 52 + i * ROW;
            boolean disabled = parent.disabledRecipes.contains(recipe.id());
            addDrawableChild(ButtonWidget.builder(recipeLabel(recipe), b -> client.setScreen(new VanillaRecipeDetailsScreen120(this, recipe)))
                    .dimensions(30, y + 1, width - 122, 18).build());
            addDrawableChild(ButtonWidget.builder(disabled ? Text.literal("Disabled").styled(s -> s.withColor(0xFF5555))
                            : Text.literal("Enabled").styled(s -> s.withColor(0x55FF55)), b -> toggle(recipe.id()))
    ...
    void requestDetails(VanillaRecipeDetailsScreen120 screen, String recipeId) {
        if (localMode) {
    ...
    private fr.zazac1.customrecipe.VanillaRecipeDetails findLocalRecipeDetails(String recipeId) {
        ...
        Identifier resourceId = Identifier.of(id.getNamespace(), "recipes/" + id.getPath() + ".json");
            ...
            for (Path archivePath : originPaths) {
                ...
                try (JarFile jar = new JarFile(normalized.toFile())) {
                    ...
                    while (entries.hasMoreElements()) {
                        var entry = entries.nextElement();
                        String path = entry.getName();
                        if (!path.startsWith("data/") || !path.contains("/recipes/") || !path.endsWith(".json")) continue;
                        int namespaceEnd = path.indexOf("/recipes/");
                        if (namespaceEnd <= "data/".length()) continue;
                        String recipeId = path.substring("data/".length(), namespaceEnd) + ":"
                                + path.substring(namespaceEnd + "/recipes/".length(), path.length() - ".json".length());
    ...
    private void collectLocalTagItems(Identifier tagId, Set<String> choices, Set<Identifier> visited) {
        ...
        Identifier tagResource = Identifier.of(tagId.getNamespace(), "tags/items/" + tagId.getPath() + ".json");
    ...
    private Text recipeLabel(VanillaRecipePage.VanillaRecipeInfo recipe) {
        return Text.literal(itemName(recipe.result()))
                .append(Text.literal("  " + shortId(recipe.id())).styled(s -> s.withColor(0xAAAAAA)));
    }
