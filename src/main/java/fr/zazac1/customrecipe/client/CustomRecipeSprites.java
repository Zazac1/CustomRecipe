...
final class CustomRecipeSprites {
    ...
    private static Identifier sprite(String name) {
        return Identifier.of(CustomRecipeMod.MOD_ID, "textures/gui/icons/" + name + ".png");
    }

    private static Identifier texture(String path) {
        return Identifier.of(CustomRecipeMod.MOD_ID, "textures/" + path + ".png");
    }

    static void draw(DrawContext context, Identifier sprite, int x, int y, int width, int height) {
        context.drawTexture(RenderPipelines.GUI_TEXTURED, sprite, x, y, 0, 0, width, height, width, height);
    }
