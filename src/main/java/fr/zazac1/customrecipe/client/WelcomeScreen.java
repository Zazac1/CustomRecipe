...
public class WelcomeScreen extends Screen {
    ...
    public WelcomeScreen(Screen returnTo) {
        super(Component.translatable("customrecipe.welcome.title"));
    ...
    protected void init() {
        ...
        // ── Description ──────────────────────────────────────────────────
        addLine(lx, py + 28, inner, Component.translatable("customrecipe.welcome.intro"), 0xCCCCCC);

        addLine(lx, py + 52, inner, Component.translatable("customrecipe.welcome.quick_add"), 0xFFEE88);

        addLine(lx, py + 72, inner, Component.translatable("customrecipe.welcome.create"), 0xFFEE88);

        // ── Boutons de navigation ────────────────────────────────────────
        addRenderableWidget(Button.builder(
                Component.translatable("customrecipe.screen.built_in"),
                b -> {
                    dismiss();
        ...
        addRenderableWidget(Button.builder(
                Component.translatable("customrecipe.welcome.create_button"),
                b -> {
                    dismiss();
                    ConfigScreen cs = configScreen();
        ...
        // ── Dismiss ──────────────────────────────────────────────────────
        addRenderableWidget(Button.builder(
                Component.translatable("customrecipe.welcome.dismiss"),
    ...
    private void addLine(int x, int y, int maxW, Component text, int color) {
        MultiLineTextWidget w = new MultiLineTextWidget(x, y,
                text.copy().withColor(color), font);
        w.setMaxWidth(maxW);
        w.setMaxRows(2);
