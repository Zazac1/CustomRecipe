...
public class CustomRecipesScreen extends Screen {
    ...
    @Override
    protected void init() {
        if (!parent.isServerManaged()) LocalRecipeConflictDetector.refresh(recipes, client);
        // ── Fills ────────────────────────────────────────────────────────
        ...
        for (int i = scroll; i < Math.min(recipes.size(), scroll + vis); i++) {
            ...
            addDrawableChild(ButtonWidget.builder(
                    corrupted ? Text.literal("Corrupted").styled(s -> s.withColor(0xFF5555))
                    : conflict ? Text.literal("Conflict").styled(s -> s.withColor(0xFFCC55))
                    ...
                    b -> {
                        ...
                        if (firstEnabledConflict(recipes.get(idx)) != null) { selectedRecipe = idx; clearAndInit(); return; }
        ...
        if (selectedRecipe >= 0 && selectedRecipe < recipes.size()) {
            ...
            MultilineTextWidget nameW = new MultilineTextWidget(
                    PAD + 4, detailY() + 4,
                    Text.literal(label).styled(s -> s.withColor(corrupted ? 0xFF7777 : 0xFFEE77)), textRenderer);
            ...
            if (corrupted) {
                ...
                MultilineTextWidget warning = new MultilineTextWidget(PAD + 4, detailY() + 76,
                        Text.literal("Missing: " + String.join(", ", missingItems(e)) + " — reinstall "
                                + (mods.isBlank() ? "the required mod" : mods) + " or delete this recipe.")
                                .styled(s -> s.withColor(0xFF7777)), textRenderer);
                warning.setMaxWidth(width - PAD * 2 - 8); warning.setMaxRows(1); addDrawableChild(warning);
                addDrawableChild(ButtonWidget.builder(Text.literal("Delete corrupted recipe").styled(s -> s.withColor(0xFF7777)), b -> {
                    recipes.remove(selected); selectedRecipe = -1; clearAndInit();
                ...
                addDrawableChild(ButtonWidget.builder(
                        ...
                        b -> { recipes.get(selected).known_by_default = !Boolean.TRUE.equals(recipes.get(selected).known_by_default); clearAndInit(); }
                ).dimensions(width - PAD - 142, detailY() + 30, 138, 18).build());
                String conflictId = firstEnabledConflict(e);
                if (conflictId != null) {
                    MultilineTextWidget warning = new MultilineTextWidget(PAD + 4, detailY() + 76,
                            Text.literal("Conflicts with " + conflictId + " — disable it to use this recipe.").styled(s -> s.withColor(0xFFCC55)), textRenderer);
                    warning.setMaxWidth(width - PAD * 2 - 8); warning.setMaxRows(1); addDrawableChild(warning);
                    addDrawableChild(ButtonWidget.builder(Text.literal("Disable conflict").styled(s -> s.withColor(0xFFCC55)), b -> {
                        if (!parent.disabledRecipes.contains(conflictId)) parent.disabledRecipes.add(conflictId); clearAndInit();
                    }).dimensions(width - PAD - 142, detailY() + 52, 138, 18).build());
                } else if (!known) {
                    String alternative = firstEnabledSameShapeRecipe(e);
                    if (alternative != null) {
                        MultilineTextWidget advice = new MultilineTextWidget(PAD + 4, detailY() + 76,
                                Text.literal("Same inputs as " + alternative + " but a different result. Set Known by default to choose it in the recipe book.")
                                        .styled(s -> s.withColor(0x77BBFF)), textRenderer);
                        advice.setMaxWidth(width - PAD * 2 - 8); advice.setMaxRows(2); addDrawableChild(advice);
                    }
                }
            }
        }
    ...
    private String firstEnabledConflict(CustomRecipeEntry entry) {
        ...
        return entry.conflicting_recipes.stream().filter(id -> id != null && !parent.disabledRecipes.contains(id)).findFirst().orElse(null);
    }

    private String firstEnabledSameShapeRecipe(CustomRecipeEntry entry) {
        if (entry.same_shape_recipes == null) return null;
        return entry.same_shape_recipes.stream().filter(id -> id != null && !parent.disabledRecipes.contains(id)).findFirst().orElse(null);
    }
