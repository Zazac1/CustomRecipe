    ...
    protected void init() {
        ...
        for (int i = scroll; i < Math.min(RECIPES.length, scroll + vis); i++) {
            ...
            MultilineTextWidget lbl = new MultilineTextWidget(
                    ...
                    Text.literal((sel ? "▸ " : "  ") + RECIPES[i][1])
                            .styled(s -> s.withColor(dis ? (sel ? 0xBBAAAA : 0x888888)
            ...
            addDrawableChild(ButtonWidget.builder(
                    dis ? Text.literal("Disabled").styled(s -> s.withColor(0xFF5555))
                        : Text.literal("Enabled").styled(s -> s.withColor(0x55FF55)),
                    b -> toggle(id)
        ...
        if (RECIPES.length > maxVisible()) {
            ...
            MultilineTextWidget hint = new MultilineTextWidget(
                    ...
                    Text.literal("↕ " + from + "-" + to + "/" + RECIPES.length).styled(s -> s.withColor(0x666666)),
                    textRenderer);
            hint.setMaxWidth(70);
            hint.setMaxRows(1);
